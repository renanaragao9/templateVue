...
function saveProduct() {
    ...
    if (product?.value.name?.trim()) {
        if (product.value.id) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Produto Criado', life: 3000 });
        }
...
function deleteSelectedProducts() {
    ...
    toast.add({ severity: 'success', summary: 'Successful', detail: 'Produtos Deletados', life: 3000 });
}
...
<template>
    <div>
            ...
            >
                <template #header>
                    <div class="flex flex-wrap gap-2 items-center justify-between">
                        <h4 class="m-0">Gerenciar Produtos</h4>
                        <IconField>
                            <InputIcon>
                ...
                <Column field="rating" header="Avaliações" sortable style="min-width: 12rem">
                    <template #body="slotProps">
        ...
        <Dialog v-model:visible="productDialog" :style="{ width: '450px' }" header="Detalhes do Produto" :modal="true">
            <div class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <small v-if="submitted && !product.name" class="text-red-500">Nome é obrigatório.</small>
                </div>
                <div>
                ...
                <div>
                    <label for="inventoryStatus" class="block font-bold mb-3">Status do Inventário</label>
                    <Select id="inventoryStatus" v-model="product.inventoryStatus" :options="statuses" optionLabel="label" placeholder="Selecione um Status" fluid></Select>
                </div>
        ...
        <Dialog v-model:visible="deleteProductsDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
            <div class="flex items-center gap-4">
                <i class="pi pi-exclamation-triangle !text-3xl" />
                <span v-if="product">Você tem certeza que deseja deletar os produtos selecionados?</span>
            </div>
            <template #footer>
